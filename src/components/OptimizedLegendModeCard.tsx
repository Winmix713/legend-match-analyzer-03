...
interface OptimizedLegendModeCardProps {
  data?: LegendModeData;
  homeTeam: string;
  awayTeam: string;
  loading?: boolean;
...
const LoadingSkeleton = memo(() => <LegendModeCardSkeleton />);
...
// Memoized premium locked state
const PremiumLockedState = memo(() => (
...
PremiumLockedState.displayName = 'PremiumLockedState';
...
// Memoized comeback analysis section
const ComebackAnalysis = memo(({ 
  data, 
  homeTeam, 
  awayTeam 
}: { 
  ...
  const memoizedComebackData = useMemo(() => ({
    homeProgress: (data.comeback_analysis.home_comebacks / 10) * 100,
    awayProgress: (data.comeback_analysis.away_comebacks / 10) * 100,
    biggestComeback: data.comeback_analysis.biggest_comeback
  }), [data.comeback_analysis]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <Progress 
          value={memoizedComebackData.homeProgress} 
          className="h-2"
        />
        ...
        <Progress 
          value={memoizedComebackData.awayProgress} 
          className="h-2"
        />
      </div>

      {memoizedComebackData.biggestComeback && (
        <div className="glass-light rounded-lg p-3">
          <p className="text-xs text-muted-foreground mb-1">Legnagyobb comeback:</p>
          <p className="text-sm font-medium text-foreground">
            {memoizedComebackData.biggestComeback.deficit} gól hátrányból
          </p>
        </div>
      )}
    </div>
...
ComebackAnalysis.displayName = 'ComebackAnalysis';

// Memoized performance trends section
const PerformanceTrends = memo(({ data }: { data: LegendModeData }) => (
  <div className="space-y-4">
    <h4 className="font-semibold text-foreground flex items-center gap-2">
...
// Memoized head to head section
const HeadToHead = memo(({ data }: { data: LegendModeData }) => (
...
const LegendModeContent = memo(({ 
  ...
  <Card className="glass p-6 animate-float">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <ComebackAnalysis data={data} homeTeam={homeTeam} awayTeam={awayTeam} />
...
LegendModeContent.displayName = 'LegendModeContent';

export const OptimizedLegendModeCard = memo(({ 
  data, 
  homeTeam, 
  awayTeam, 
  loading = false, 
  ...
  const shouldShowPremiumLock = useMemo(() => !isPremium, [isPremium]);
  const shouldShowContent = useMemo(() => {
    return !loading && !error && isPremium && data !== undefined;
  }, [loading, error, isPremium, data]);
...
OptimizedLegendModeCard.displayName = 'OptimizedLegendModeCard';